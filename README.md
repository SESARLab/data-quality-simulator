...
```bash
make run CONFIG_FILE_PATH=config-files/inmates_enriched_10k.json SIMULATOR_ARGS=[...]
```

If the environment already have Swift installed (e.g. when you are developing using VSCode devcontainer feature):

```bash
make run IS_DEVCONTAINER=true CONFIG_FILE_PATH=config-files/inmates_enriched_10k.json SIMULATOR_ARGS=[...]
```
