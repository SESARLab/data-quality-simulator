...
class Simulation {
    ...
    private func internalRun(on data: PythonObject) throws -> StatsCalculator {
        ...
        for (index, window) in windows.enumerated() {
            ...
            self.logger.info("Simulator status - samplings processed: \(self.timeMonitor.getDoneSampling()) / \(self.timeMonitor.totalSamplings), completion percent: \(timeMonitor.getCompletionPercent())%, estimated time left: \(timeMonitor.getEstimatedTimeLeft())")
        }
